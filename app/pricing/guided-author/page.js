...
const INCLUDED = [
  ...
  { icon: 'ph ph-chats', label: 'Expert Writing Coach', detail: 'Structured guidance and feedback throughout your writing process' },
  { icon: 'ph ph-users-three', label: 'Peer Author Community', detail: 'Connect with fellow Readora authors for support and accountability' },
  ...
  { icon: 'ph ph-package', label: '50 Paperback or 25 Hardcover Author Copies', detail: 'Author copies included. Unlimited extras at printer cost.' },
...
<h4>Timeline</h4>
<p>The minimum time-to-publish for this experience is 12 months from your kickoff call. That's as fast as we can move. It's completely normal for authors to take closer to 18 months, so that's what we recommend you budget.</p>
...
<p><strong>North Star and Book Roadmap Sessions:</strong> You will have 2 calls with your Book Coach focused on positioning your book in the market and defining your North Star. You will then have up to 4 additional calls to build a detailed chapter-by-chapter outline (your Book Roadmap).</p>
<p><strong>Book Coaching and Review Sessions:</strong> During the manuscript writing phase, you will have 2 coaching sessions with your Book Coach to work through any challenges, get feedback on your progress, and stay on track.</p>
<p><strong>Digital Guided Author Course Access:</strong> You will receive access to our comprehensive Guided Author writing course, delivered in modules that walk you through the entire writing process step by step.</p>
<p><strong>Guided Author Community Access:</strong> You will join our Guided Author community where you can interact with other authors going through the process, connect with coaches, and participate in weekly accountability sessions.</p>
<p><strong>"Vomit Draft" Evaluation and Review:</strong> Once you complete your first rough draft, your Book Coach will review it and provide a 60-minute feedback call to guide your revisions.</p>
<p><strong>Full Manuscript Evaluation and Structural Edit:</strong> After your revisions, our Executive Editor will do a complete review of your manuscript, including a developmental edit with structural feedback. You will have up to 3 calls with your editor.</p>
...
<p><strong>Copyright & ISBN Registration:</strong> Your manuscript will be copyrighted with the U.S. Copyright Office and registered in Bowker's Books In Print database with all necessary ISBNs and barcodes.</p>
...
<p><strong>Author Copies:</strong> We will provide 50 paperback copies or 25 hardcover copies at no extra charge.</p>
<p><strong>Audiobook Creation and Distribution:</strong> We will facilitate the creation and publication of an audiobook version, available on Amazon, Audible, Apple Books, Scribd, Google Play, and more.</p>
...
<p><strong>The Opt Out Clause:</strong> If our Executive Editor determines we cannot publish a manuscript in its current form, and you don't want to revise it, both parties can opt out without penalty.</p>
<p><strong>Total Length Clause:</strong> Our price works for books up to 50,000 words. If the manuscript is over 50k words, we charge an additional $100 per 1,000 words.</p>
<h4>Legal Terms</h4>
...
function fmtMonthly(n) { return '$' + n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }

export default function PricingGuidedAuthor() {
  const [step, setStep] = useState(1)
  const [selectedAddons, setSelectedAddons] = useState({})
  const [imprint, setImprint] = useState('lioncrest')
  const [additionalAuthor, setAdditionalAuthor] = useState(false)
  ...
  const addonTotal = useMemo(() => {
    let t = 0
    for (const a of [...ADDONS.flatMap(g => g.items), ...MORE_ADDONS]) {
      if (selectedAddons[a.id]) t += a.price
    }
    return t
  }, [selectedAddons])

  const total = BASE_PRICE + imprintCost + addonTotal + (additionalAuthor ? 3500 : 0) - (returningAuthor ? 2000 : 0)
  ...
  const handleEmailQuote = async () => {
    ...
    try {
      const res = await fetch(API_BASE + '/api/email-quote', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: quoteEmail, tier: 'Readora Guided Author', totalAmount: total, monthlyPayment: monthly, numMonths: NUM_MONTHS, addons: Object.keys(selectedAddons).filter(k => selectedAddons[k]), basePrice: BASE_PRICE }) })
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await fetch(API_BASE + '/api/create-checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tier: 'Readora Guided Author', addons: Object.keys(selectedAddons).filter(k => selectedAddons[k]), totalAmount: total, monthlyPayment: monthly, numMonths: NUM_MONTHS, authorName: signName, email: signEmail, phone: signPhone, address: signAddress, city: signCity, state: signState, zip: signZip, agreementSignature: signSig, agreementTimestamp: new Date().toISOString() }) })
      const data = await res.json()
  ...
  const MktCard = ({ item }) => (
    <label className={`pub-mkt-card${selectedAddons[item.id] ? ' is-selected' : ''}`}>
      ...
      <div className="pub-mkt-card__body">
        ...
        {item.details && <ul className="pub-mkt-card__details">{item.details.map((d, i) => <li key={i}>{d}</li>)}</ul>}
        ...
        <div className="pub-layout">
          <div className="pub-main">
            ...
            {step === 1 && (
              <div>
                <div className="pub-hero">
                  <div className="pub-hero__badge">Your Guided Writing &amp; Publishing Package</div>
                  <h1 className="pub-hero__title">Readora Guided Author</h1>
                  <p className="pub-hero__subtitle">Write your book with expert coaching from Readora. You write the manuscript with structured guidance, and we handle everything else: editing, design, audiobook, and global publication. You retain 100% of your royalties.</p>
                ...
                <button className={`pub-customize-toggle${showCustomize ? ' open' : ''}`} onClick={() => setShowCustomize(v => !v)}>
                  <span>Customize Your Package</span><span className="arrow">▼</span>
                </button>
                {showCustomize && (
                  <div className="pub-card" style={{ marginTop: 0 }}>
                    ...
                    <div className="pub-card-section-label" style={{ marginTop: '18px' }}>Additional Authors</div>
                    <p style={{ fontSize: '13px', color: '#a0a0a0', marginBottom: '12px' }}>Add co-authors to your project.</p>
                    <label className="pub-check">
                      <input type="checkbox" checked={additionalAuthor} onChange={e => setAdditionalAuthor(e.target.checked)} />
                      <div className="pub-check__info">
                        <div className="pub-check__name">Additional Author</div>
                        <div className="pub-check__desc">Add a co-author to your project</div>
                      </div>
                      <div className="pub-check__price" style={{ color: '#e02d2d' }}>+$3,500</div>
                    </label>
                    ...
                    <label className="pub-check">
                      ...
                      <div className="pub-check__price">−$2,000</div>
                    </label>
                  </div>
                )}
            ...
            {step === 2 && (
              <div className="pub-card">
                ...
                <div className="pub-agreement" dangerouslySetInnerHTML={{ __html: AGREEMENT_TEXT + `
                  ...
                  <p>Your investment is paid over <strong>${NUM_MONTHS}</strong> months at <strong>${monthlyStr}</strong>/month. A $1,000 deposit is due at signing and will be deducted from your first monthly payment. There is a 3% processing fee for all credit card payments. If we're still working on your book beyond 18 months, payments continue at $1,500/month. Full payment is required before publishing.</p>
                  <table>
                    <tbody>
                      <tr><td>Readora Guided Author Services</td><td style="text-align:right;font-weight:600;">$44,000</td></tr>
                      ${imprintCost > 0 ? `<tr><td>Custom Imprint</td><td style="text-align:right;font-weight:600;">${fmt(imprintCost)}</td></tr>` : ''}
                      ${additionalAuthor ? `<tr><td>Additional Author</td><td style="text-align:right;font-weight:600;">+$3,500</td></tr>` : ''}
                      ${returningAuthor ? `<tr><td>Returning Author Discount</td><td style="text-align:right;color:#4ade80;font-weight:600;">-$2,000</td></tr>` : ''}
                      ${allAddons.filter(a => selectedAddons[a.id]).map(a => `<tr><td>${a.name}</td><td style="text-align:right;font-weight:600;">${fmt(a.price)}</td></tr>`).join('')}
            ...
            {step === 3 && (
              <div className="pub-card">
                ...
                <div className="pub-sign-field"><label>Full Legal Name</label><input type="text" placeholder="Your full name as it appears on legal documents" value={signName} onChange={e => setSignName(e.target.value)} /></div>
                <div className="pub-sign-field"><label>Email Address</label><input type="email" placeholder="your@email.com" value={signEmail} onChange={e => setSignEmail(e.target.value)} /></div>
                <div className="pub-sign-field"><label>Phone Number</label><input type="tel" placeholder="(555) 555-5555" value={signPhone} onChange={e => setSignPhone(e.target.value)} /></div>
                <div className="pub-sign-field"><label>Mailing Address</label><input type="text" placeholder="Street address" value={signAddress} onChange={e => setSignAddress(e.target.value)} /></div>
                <div className="pub-sign-row">
                  <div className="pub-sign-field" style={{ flex: 2 }}><label>City</label><input type="text" placeholder="City" value={signCity} onChange={e => setSignCity(e.target.value)} /></div>
                  <div className="pub-sign-field" style={{ flex: 1 }}><label>State</label><input type="text" placeholder="State" value={signState} onChange={e => setSignState(e.target.value)} /></div>
                  <div className="pub-sign-field" style={{ flex: 1 }}><label>Zip Code</label><input type="text" placeholder="Zip" value={signZip} onChange={e => setSignZip(e.target.value)} /></div>
                </div>
                <label className="pub-sign-checkbox"><input type="checkbox" checked={signTerms} onChange={e => setSignTerms(e.target.checked)} />I have read and agree to the terms of this agreement</label>
                <label className="pub-sign-checkbox"><input type="checkbox" checked={signPayment} onChange={e => setSignPayment(e.target.checked)} />I agree to the payment schedule shown in the summary</label>
                <label className="pub-sign-checkbox"><input type="checkbox" checked={signElectronic} onChange={e => setSignElectronic(e.target.checked)} />I consent to conduct this transaction electronically</label>
                ...
                <div style={{ fontSize: '12px', color: '#a0a0a0', marginTop: '12px', textAlign: 'center' }}>Signed on: {new Date().toLocaleString()} · IP recorded automatically</div>
            ...
            {step === 4 && (
              <div className="pub-card">
                ...
                <div className="pub-pay-summary">
                  ...
                  <div className="pay-note" style={{ marginTop: '8px' }}>This $1,000 will be deducted from your first monthly payment of {monthlyStr}/mo.</div>
                  <div className="pay-note pay-note--green" style={{ marginTop: '8px' }}>Pay your deposit and we'll start your book process immediately.</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <button className={`pub-btn pub-btn--primary pub-btn--lg${payBtnLoading ? ' pub-btn--disabled' : ''}`} onClick={handleCheckout}>
                    {payBtnLoading ? 'Creating checkout session...' : 'Pay $1,000 Deposit'}
                  </button>
                  <p style={{ fontSize: '12px', color: '#a0a0a0', marginTop: '12px' }}>You will be redirected to Stripe's secure checkout page.<br />Powered by Stripe. Your card information never touches our servers.</p>
                </div>
                <div style={{ textAlign: 'center', marginTop: '16px' }}>
                  ...
                  <p style={{ fontSize: '11px', color: '#a0a0a0', marginTop: '6px', opacity: 0.7 }}>Your book process won't begin until we receive your first payment or ACH information.</p>
                </div>
                <div className="pub-consult-cta">
          ...
          <div className="pub-sidebar">
            <div className="pub-summary">
              ...
              <div className="pub-summary__line"><span className="label">Readora Guided Author</span><span className="value">{fmt(BASE_PRICE)}</span></div>
              ...
              {additionalAuthor && <div className="pub-summary__line"><span className="label">Additional Author</span><span className="value" style={{ color: '#e02d2d' }}>+$3,500</span></div>}
              {returningAuthor && <div className="pub-summary__line"><span className="label">Returning author discount</span><span className="value" style={{ color: '#4ade80' }}>-$2,000</span></div>}
              {allAddons.filter(a => selectedAddons[a.id]).map(a => (
                <div key={a.id} className="pub-summary__line"><span className="label">{a.name}</span><span className="value">+{fmt(a.price)}</span></div>
              ))}
              <div className="pub-summary__total"><span>Total Investment</span><span className="value">{fmt(total)}</span></div>
              ...
              <div className="pub-email-quote">
                ...
                {quoteStatus && (
                  <div className="pub-email-quote__status" style={{ color: quoteStatus.ok === true ? '#4ade80' : quoteStatus.ok === false ? '#f87171' : '#a0a0a0' }}>{quoteStatus.msg}</div>
                )}
              </div>
            </div>
          </div>
